...
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    // Check mode change
    {
        ...
        if(mpTracker->mbOnlyTracking && mpTracker->mState == Tracking::OK)
            {
                cout << setprecision(4);
                //cout << "Localization!!\n";
                //cv::putText(mpFrameDrawer->DrawFrame(),"Hello",cv::Point(25, 45), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 200, 200), 4);
                
                //cout << "1" << endl;
                cv::Mat t3 = mpTracker->mCurrentFrame.GetCameraCenter();
                if( ((t3.at<float>(0) - (-0.2563184)) < 0.15 && (t3.at<float>(0) - (-0.2563184)) > -0.15) &&
                        t3.at<float>(1) - (-0.02205789) < 0.015 && (t3.at<float>(1) - (-0.02205789) > -0.015)   )
                    mpViewer->index = 1;
                else
                    mpViewer->index = 0;
                cout << setprecision(7) << t3.at<float>(0) << "  " << t3.at<float>(1) << endl;
                
                
               
               /// CompareTrajectory(-0.9060481, 0.1466489, 0.2614615);
            }
        else
            mpViewer->index = 0;
...
void System::CompareTrajectory(float x, float y, float z)
{
    vector<KeyFrame*> vpKFs = mpMap->GetAllKeyFrames();
    sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
       ...
       // pKF->SetPose(pKF->GetPose()*Two);
        ...
        cout << setprecision(7);
        if(x == t.at<float>(0) && y == t.at<float>(1) && z == t.at<float>(2))
            cout << "Correct" << endl; 
    }
}
